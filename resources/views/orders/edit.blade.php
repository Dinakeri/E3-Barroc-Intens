<x-layouts.dashboard :title="'Sales Dashboard - nieuwe bestelling'">
    ...
    <div class="mt-10">
        <div class=" mb-16">
            ...
            <flux:text class="mt-2">Vul het onderstaande formulier in om een nieuwe bestelling aan te maken.
            </flux:text>
        </div>
        ...
        <div class="border rounded-lg border-zinc-900 dark:border-zinc-400">
            ...
            <form action="{{ route('orders.update', $order) }}" method="POST" x-data="orderForm(@js($products), @js($existingItems), {{ $order->customer_id }})" class="p-6">
                @csrf
                @method('PUT')
                    ...
                    <div x-data="{
                        ...
                        <flux:button type="button" @click="open = !open" x-cloak
                            ...
                            <div class="w-full flex justify-between">
                                <span
                                    x-text="selected ? selected.name + ' (' + selected.email + ')'
                                    : customers.find(c => c.id == customerId)?.name || 'Kies een klant'"></span>
                    ...
                    <flux:fieldset>
                        ...
                        <flux:select name="status" required>
                            ...
                            <option value="completed"
                                {{ old('status', $order->status) == 'completed' ? 'selected' : '' }}>Voltooid</option>
                            <option value="cancelled"
                                {{ old('status', $order->status) == 'cancelled' ? 'selected' : '' }}>Geannuleerd
                            </option>
                        </flux:select>
                    </flux:fieldset>
                </div>
    ...
    <script>
        function orderForm(products, existingItems = [], customerId = null) {
            return {
                // 🧠 STATE
                products: products,
                selectedProductId: '',
